...
def get_tracks_with_all_valied_timestamps(track_infos, info):
    trajs = track_infos['trajs']                     # Shape: [num_tracks, num_timesteps, num_features]
    indices = np.array(info['tracks_to_predict']['track_index'])

    # Extract the relevant trajectories
    selected_trajs = trajs[indices, 10:, -1]         # Shape: [num_selected_tracks, timesteps_after_10]

    # Sum the valid timestamps across time axis
    valid_counts = selected_trajs.sum(axis=1)        # Shape: [num_selected_tracks]

    # Filter indices where count == 81
    indices = indices[valid_counts == 81]
...
def create_infos_from_protos(raw_data_path, output_path, num_workers=1):
    ...
    val_infos = get_infos_from_protos(
        data_path=os.path.join(raw_data_path, 'training'),
        output_path=os.path.join(output_path, 'processed_scenarios_training_ego_all_valid_timestamps'),
        num_workers=num_workers
    )
    val_filename = os.path.join(output_path, 'processed_scenarios_training_ego_all_valid_timestamps_infos.pkl')
    with open(val_filename, 'wb') as f:
        pickle.dump(val_infos, f)
    print('----------------Waymo info val file is saved to %s----------------' % val_filename)
