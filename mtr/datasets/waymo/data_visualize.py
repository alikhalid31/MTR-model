...
def plot_scenario(track_infos, map_infos, dynamic_map_infos , output_path):
...
def process_waymo_data_with_scenario_proto(data_file, output_path=None):
    ...
    #     scenario.ParseFromString(bytes(data.numpy()))
        ...
        # Plot map centerlines
        plt.figure(figsize=(10, 10))
        for polyline in scenario.map_features:
            if polyline.WhichOneof('feature_data') == 'lane':
                lane = polyline.lane
                x = [pt.x for pt in lane.polyline]
                y = [pt.y for pt in lane.polyline]
                plt.plot(x, y, color='gray', linewidth=0.5)

        # Plot agent trajectories (past & future)
        for track in scenario.tracks:
            if track.object_type == 1:  # 1: VEHICLE
                x_past = [s.center_x for s in track.past_trajectory]
                y_past = [s.center_y for s in track.past_trajectory]
                x_future = [s.center_x for s in track.future_trajectory]
                y_future = [s.center_y for s in track.future_trajectory]

                plt.plot(x_past, y_past, 'b-', alpha=0.5)
                plt.plot(x_future, y_future, 'r--', alpha=0.5)
        ...
        # plt.show()
        plt.tight_layout()
        output_file = os.path.join(output_path, f'sample_{scenario.scenario_id}.png')
...
def create_infos_from_protos(raw_data_path, output_path, num_workers=1):
    ...
    val_infos = get_infos_from_protos(
        ...
        output_path=os.path.join(output_path, 'processed_scenarios_debuging_viz'),
        num_workers=num_workers
    )
    # val_filename = os.path.join(output_path, 'processed_scenarios_debuging_infos.pkl')
