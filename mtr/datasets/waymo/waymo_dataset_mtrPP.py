...
class WaymoDataset(DatasetTemplate):
    ...
    def create_scene_level_data(self, index):
        ...
        timestamps = np.array(info['timestamps_seconds'][:current_time_index + 1], dtype=np.float32) # hsitory timestamps
        ...
        # check if the center objects are valid at current timestamp and update the track_index_to_predict.
        # Also get the feautes of valid center objects.
        ...
        (obj_trajs_data, obj_trajs_mask, obj_trajs_pos, obj_trajs_heading, obj_trajs_last_pos, obj_trajs_future_state, obj_trajs_future_mask, center_gt_trajs,
            center_gt_trajs_mask, center_gt_final_valid_idx,
            track_index_to_predict_new, sdc_track_index_new, obj_types, obj_ids) = self.create_agent_data(
        ...
        ret_dict = {
            ...
            'obj_trajs_heading': obj_trajs_heading,
        ...
        if not self.dataset_cfg.get('WITHOUT_HDMAP', False):
            ...
            map_polylines_data, map_polylines_mask, map_polylines_center, map_polylines_heading = self.create_map_data(
                map_infos=info['map_infos'])   
            # (num_polylines, num_points_each_polyline, 9), (num_polylines, num_points_each_polyline)
            # (num_polylines, num_points_each_polyline, 2), (num_polylines, num_points_each_polyline,1)

            ret_dict['map_polylines'] = map_polylines_data
            ret_dict['map_polylines_mask'] = (map_polylines_mask > 0)
            ret_dict['map_polylines_center'] = map_polylines_center
            ret_dict['map_polylines_heading'] = map_polylines_heading
    ...
    def create_agent_data(
        ...
        # obj_trajs_data (num_objects, num_timestamps_past, num_attrs + embeddings), zero at invalid timestamp
        # obj_trajs_mask (num_objects, num_timestamps_past), mask for invalid timestamps
        # obj_trajs_future_state (num_objects, num_timestamps_future, 4):  [x, y, vx, vy]
        # obj_trajs_future_mask (num_objects, num_timestamps_future):
        obj_trajs_data, obj_trajs_mask, obj_trajs_pos, obj_trajs_heading, obj_trajs_future_state, obj_trajs_future_mask = self.generate_local_trajs_for_agents(
        ...
        # Select only the predicted agents
        center_gt_trajs = obj_trajs_future_state[track_index_to_predict]        # (num_center_objects, num_future_timestamps, 4)
        center_gt_trajs_mask = obj_trajs_future_mask[track_index_to_predict]    # (num_center_objects, num_future_timestamps)

        # Zero-out invalid steps (optional — depends on your loss code)
        ...
        obj_trajs_mask = obj_trajs_mask[valid_past_mask]  # (num_objects (filtered), num_timestamps)
        obj_trajs_data = obj_trajs_data[valid_past_mask]  # (num_objects (filtered), num_timestamps, C)
        obj_trajs_future_state = obj_trajs_future_state[valid_past_mask]  # (num_objects (filtered), num_timestamps_future, 4):  [x, y, vx, vy]
        obj_trajs_future_mask = obj_trajs_future_mask[valid_past_mask]  # (num_objects, num_timestamps_future):
        obj_types = obj_types[valid_past_mask]
        obj_ids = obj_ids[valid_past_mask]

        # updating the track_index_to_predict and sdc_track_index as some of the objects are filtered out in the previous step
        ...
        # maybe we have to change 1 to 0 for all these asserts
        assert obj_trajs_future_state.shape[0] == obj_trajs_data.shape[0]
        assert len(obj_types) == obj_trajs_future_mask.shape[0]
        assert len(obj_ids) == obj_trajs_future_mask.shape[0]

        # generate the final valid position of each object
        obj_trajs_pos_centered = obj_trajs_data[:, :, 0:3]
        num_objects, num_timestamps, _ = obj_trajs_pos_centered.shape
        ...
        for k in range(num_timestamps):
            ...
            obj_trajs_last_pos[cur_valid_mask] = obj_trajs_pos_centered[:, k, :][cur_valid_mask]

        center_gt_final_valid_idx = np.zeros((len(track_index_to_predict)), dtype=np.float32)
        for k in range(center_gt_trajs_mask.shape[1]):
            cur_valid_mask = center_gt_trajs_mask[:, k] > 0
            center_gt_final_valid_idx[cur_valid_mask] = k


        return (obj_trajs_data, obj_trajs_mask > 0, obj_trajs_pos, obj_trajs_heading, obj_trajs_last_pos,
    ...
    @staticmethod
    def transform_trajs_to_local_coords(obj_trajs, center_xyz, center_heading, heading_index, rot_vel_index=None):
        """
        Transforms each agent's trajectory to its own local coordinate system.
        The local frame origin is the last valid position of the agent, and the x-axis is aligned with its velocity direction.

        Args:
            obj_trajs (num_objects, num_timestamps, num_attrs):
                - First 3 attrs must be [x, y, z]
                - Last attr is expected to be a valid flag
            center_xyz (num_objects, 3): [x, y, z] at last valid timestamp
            center_heading (num_objects): heading angle at last valid timestamp
            heading_index: index of heading angle in obj_trajs (used to rotate angles)
            rot_vel_index: index pair [vx_idx, vy_idx] for velocity components (optional)

        Returns:
            obj_trajs_local: (num_objects, num_timestamps, num_attrs) in local frame
        """
        obj_trajs = obj_trajs.clone()
        num_objects, num_timestamps, num_attrs = obj_trajs.shape

        # Center the positions
        obj_trajs[:, :, 0:3] = obj_trajs[:, :, 0:3] - center_xyz[:, None, :]

        # Rotate coordinates to align with heading
        coords = obj_trajs[:, :, 0:2].clone().view(num_objects, -1, 2)
        coords_rot = common_utils.rotate_points_along_z(coords, -center_heading)
        obj_trajs[:, :, 0:2] = coords_rot.view(num_objects, num_timestamps, 2)

        # Rotate heading angle if present
        obj_trajs[:, :, heading_index] -= center_heading[:, None]

        # Rotate velocity direction if needed
        if rot_vel_index is not None:
            vel = obj_trajs[:, :, rot_vel_index[0]:rot_vel_index[1]+1].view(num_objects, -1, 2)
            vel_rot = common_utils.rotate_points_along_z(vel, -center_heading)
            obj_trajs[:, :, rot_vel_index[0]:rot_vel_index[1]+1] = vel_rot.view(num_objects, num_timestamps, 2)

        return obj_trajs

    @staticmethod
    def get_position_and_heading_at_last_valid_timestamp(obj_trajs, heading_index):
        """
        Get the position of each object at its last valid timestamp.
        Args:
            obj_trajs (num_objects, num_timestamps, num_attrs):
                - First 3 attrs must be [x, y, z]
                - Last attr is expected to be a valid flag
        ...
        valid_mask = obj_trajs[:, :, -1] > 0  # shape: (N, T)

        # Find the last valid timestep for each object
        last_valid_idx = valid_mask.float().cumsum(dim=1).argmax(dim=1)  # shape: (N,)

        # Gather last valid position [x, y, z]
        center_xyz = torch.zeros((num_objects, 3), dtype=torch.float32, device=obj_trajs.device)
        for i in range(num_objects):
            center_xyz[i] = obj_trajs[i, last_valid_idx[i], 0:3]

        heading = torch.zeros((num_objects,), dtype=torch.float32, device=obj_trajs.device)
        for i in range(num_objects):
            heading[i] = obj_trajs[i, last_valid_idx[i], heading_index]


        return center_xyz, heading  # shape: (num_objects, 3)

    def generate_local_trajs_for_agents(self, center_objects, obj_trajs_past, obj_types, center_indices, sdc_index, timestamps, obj_trajs_future):
        """[summary]
        ...
        Returns:
            ret_obj_trajs (num_objects, num_timestamps, num_attrs):
            ret_obj_valid_mask (num_objects, num_timestamps):
            ret_obj_trajs_future (num_objects, num_timestamps_future, 4):  [x, y, vx, vy]
            ret_obj_valid_mask_future (num_objects, num_timestamps_future):
        """
        ...
        # Gather last valid position [x, y, z] and heading
        # Need to find the latest valid timestamp for each object as it is the origin of the local frame.
        center_xyz, center_heading = self.get_position_and_heading_at_last_valid_timestamp(obj_trajs=obj_trajs_past, heading_index=6 )  
        

        # transform coordinates to the local objects
        obj_trajs = self.transform_trajs_to_local_coords(
        ...
        # === One-hot object type encoding ===
        object_onehot_mask = torch.zeros((num_objects, num_timestamps, 5), dtype=torch.float32, device=obj_trajs.device)
        object_onehot_mask[obj_types == 'TYPE_VEHICLE', :, 0] = 1
        object_onehot_mask[obj_types == 'TYPE_PEDESTRIAN', :, 1] = 1  # FIXED typo from PEDESTRAIN
        object_onehot_mask[obj_types == 'TYPE_CYCLIST', :, 2] = 1
        object_onehot_mask[center_indices, :, 3] = 1  # mark center agent
        object_onehot_mask[sdc_index, :, 4] = 1     # mark SDC

        # === Time embedding ===
        object_time_embedding = torch.zeros((num_objects, num_timestamps, num_timestamps + 1), dtype=torch.float32, device=obj_trajs.device)
        time_idx = torch.arange(num_timestamps, device=obj_trajs.device)
        object_time_embedding[:, time_idx, time_idx] = 1
        object_time_embedding[:, time_idx, -1] = timestamps

        # === Heading embedding ===
        object_heading_embedding = torch.zeros((num_objects, num_timestamps, 2), dtype=torch.float32, device=obj_trajs.device)
        object_heading_embedding[:, :, 0] = torch.sin(obj_trajs[:, :, 6])
        object_heading_embedding[:, :, 1] = torch.cos(obj_trajs[:, :, 6])

        # === Velocity and Acceleration ===
        vel = obj_trajs[:, :, 7:9]
        vel_pre = torch.roll(vel, shifts=1, dims=1)
        acce = (vel - vel_pre) / 0.1
        acce[:, 0, :] = acce[:, 1, :]  # fix first frame

        # === Final feature tensor ===
        ret_obj_trajs = torch.cat((
            obj_trajs[:, :, 0:6],           # [x, y, z, dx, dy, dz]
            object_onehot_mask,
        ...
        ret_obj_valid_mask = obj_trajs[ :, :, -1]  # (num_objects, num_timestamps)  # TODO: CHECK THIS, 20220322
        ...
        obj_trajs_future = self.transform_trajs_to_local_coords(
        ...
        ret_obj_valid_mask_future = obj_trajs_future[:, :, -1]  # (num_objects, num_timestamps_future)  # TODO: CHECK THIS, 20220322
        ret_obj_trajs_future[ret_obj_valid_mask_future == 0] = 0

        return ret_obj_trajs.numpy(), ret_obj_valid_mask.numpy(), center_xyz.numpy(), center_heading.numpy(),  ret_obj_trajs_future.numpy(), ret_obj_valid_mask_future.numpy()
    ...
    def create_map_data(self, map_infos):
        ...
        polyline_centers = batch_polylines[:, :, 0:3].sum(dim=1) / torch.clamp_min(batch_polylines_mask.sum(dim=1).float()[:, None], min=1.0)
